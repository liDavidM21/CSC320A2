...
class Inpainting:
    ...
    def readImage(self, fileName, key):
        ...
        if(key not in self._images):
            ...
            if(key == 'alpha'):
                self._images[key] = cv.cvtColor(self._images[key], cv.COLOR_BGR2GRAY)
            elif(key == 'source'):
                self._images[key] = cv.cvtColor(self._images[key], cv.COLOR_BGRA2BGR)
        #########################################
        return success, msg
...
success, errorMessage = exampleBasedInpainting(self)
        ...
        if(self._images['alpha'] is None or self._images['source'] is None):
            success = False
            msg = 'No image avaliable'
            return success, msg
        elif(self._images['alpha'].shape[0:2] != self._images['source'].shape[0:2]):
            success = False
            msg = 'Not same size'
            return success, msg
        elif(len(self._images['alpha'].shape) != 2 or self._images['source'].shape[2] != 3):
            success = False
            msg = 'Not all color or greyscale'
            return success, msg
        else:
            success = True
