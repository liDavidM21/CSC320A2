...
def computeGradient(psiHatP=None, inpaintedImage=None, filledImage=None):
    ...
    grey = cv.cvtColor(psiHatP.pixels(), cv.COLOR_BGR2GRAY)
    x = cv.Sobel(grey, cv.CV_64F, 1, 0)
    y = cv.Sobel(grey, cv.CV_64F, 0, 1)
    total = np.multiply(x,x) + np.multiply(y,y)
    pos = np.unravel_index(np.argmax(total), total.shape)
    dx = x[pos[0]][pos[1]]
    dy = y[pos[0]][pos[1]]
    #########################################
    return dy,dx
...
#         An OpenCV image of type uint8 that contains a value of 255
#         for every pixel in image I whose color is known (ie. either
#         a pixel that was not masked sinitially or a pixel that has
#         already been inpainted), and 0 for all other pixels
#    fillFront:
